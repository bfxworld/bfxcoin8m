...
for i in ${svg_stroke[@]}; do
  ...
  $CONVERT -geometry "$PIX_SIZE" -background none "$TMP_FILE" \
                    "${PIXMAP_FOLDER}/bfxcoin${PIX_SIZE}.png"
  $CONVERT -geometry "$PIX_SIZE" -background none "$TMP_FILE" \
                    "${PIXMAP_FOLDER}/bfxcoin${PIX_SIZE}.bmp"
  $CONVERT -geometry "$PIX_SIZE" -background none "$TMP_FILE" \
                    "${PIXMAP_FOLDER}/bfxcoin${PIX_SIZE}.xpm"
  ico_list_bmp="${ico_file_list} ${PIXMAP_FOLDER}/bfxcoin${PIX_SIZE}.bmp"
  ico_list_png="${ico_file_list} ${PIXMAP_FOLDER}/bfxcoin${PIX_SIZE}.png"
done

$CONVERT ${ico_list_bmp} "${PIXMAP_FOLDER}/bfxcoin.ico"   # png ico is only Windows7+
[ x"$PNG2ICNS" != "x" ] && $PNG2ICNS "${PIXMAP_FOLDER}/bfxcoin.icns" ${ico_list_png}
