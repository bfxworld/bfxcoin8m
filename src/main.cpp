...
static string strMineMessage = _("Warning: Mining suspended due to insufficient wallet balance. you must have 1 million coins to be able to do POS Mining.");
...
bool BitcoinMiner(CWallet *pwallet, bool fProofOfStake, uint256 * minedBlock, uint64 nTimeout)
{
    ...
    while (minedBlock || fGenerateBitcoins || fProofOfStake)
    {
		...
		if (!fProofOfStake)
		{
			printf("Wallet Balance %s\n", FormatMoney(pwallet->GetBalance()).c_str());
			printf("Block height %d\n",pindexBest->nHeight);
			
			if (pwallet->GetBalance()<100000000 && pindexBest->nHeight >300)
			{
			   printf("BFX Miner is going to sleep now");
			   strMintWarning = strMintMessage;
			   Sleep(1000);
			}
		}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
	...
	int64 minBal = 0;
	minBal = 1000000000000;
	if (pwallet->IsLocked())
	{
		
	   printf("BFX Miner is terminating as the wallet is locked.");
       return;
	}
    if (pindexBest->nHeight >= 300)
    {	
		int64 diff = pwallet->GetBalance() - minBal;
		printf("Diff Balance is %s\n", FormatMoney(diff).c_str());
		if (diff<=0)
		{
			printf("BFX Miner is hibernating now");
			return;
		}
		else
		{
			printf("hibernating returns in else case");
